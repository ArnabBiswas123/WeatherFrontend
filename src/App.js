...
function App() {
  ...
  const fetchWeatherBylat = async () => {
    ...
    if (latlon.lat !== "" && latlon.lon !== "") {
      const result = await fetch(
        `https://weatherbackend-phez.onrender.com/api/v1/weather/getweatherdatabylat/${latlon.lat}/${latlon.lon}/${units}`
      );
  ...
  const fetchweather = async () => {
    const result = await fetch(
      `https://weatherbackend-phez.onrender.com/api/v1/weather/getweatherdata/${query}/${units}`
    );
    const data = await result.json();
    if (data.success === false) {
      toast({
